...
import 'package:thunder/features/event/domain/usecases/send_join_request_usecase.dart';
import 'package:thunder/features/event/domain/repositories/event_repository.dart';
import 'package:thunder/core/errors/failures.dart';

import 'send_join_request_usecase_test.mocks.dart';

/// Mock classes için annotation
@GenerateMocks([EventRepository])
void main() {
  late SendJoinRequestUseCase useCase;
  late MockEventRepository mockRepository;

  setUp(() {
    mockRepository = MockEventRepository();
    useCase = SendJoinRequestUseCase(mockRepository);
  });

  group('SendJoinRequestUseCase', () {
    const testEventId = 'event-123';
    const testUserId = 'user-123';

    test('should return Right(void) when request is sent successfully', () async {
      // Arrange
      when(mockRepository.sendJoinRequest(testEventId, testUserId))
      ...
      verify(mockRepository.sendJoinRequest(testEventId, testUserId)).called(1);
    ...
    test('should return Left(ValidationFailure) when eventId is empty', () async {
      ...
      verifyNever(mockRepository.sendJoinRequest(any, any));
    });
    ...
    test('should return Left(ServerFailure) when repository returns failure', () async {
      // Arrange
      final failure = ServerFailure('Send join request failed');
      when(mockRepository.sendJoinRequest(testEventId, testUserId))
      ...
      expect(result.left.message, 'Send join request failed');
      verify(mockRepository.sendJoinRequest(testEventId, testUserId)).called(1);
    });
  });
}
