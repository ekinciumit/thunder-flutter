...
import 'package:thunder/features/event/domain/usecases/reject_join_request_usecase.dart';
import 'package:thunder/features/event/domain/repositories/event_repository.dart';
import 'package:thunder/core/errors/failures.dart';

import 'reject_join_request_usecase_test.mocks.dart';

/// Mock classes için annotation
@GenerateMocks([EventRepository])
void main() {
  late RejectJoinRequestUseCase useCase;
  late MockEventRepository mockRepository;

  setUp(() {
    mockRepository = MockEventRepository();
    useCase = RejectJoinRequestUseCase(mockRepository);
  });

  group('RejectJoinRequestUseCase', () {
    const testEventId = 'event-123';
    const testUserId = 'user-123';

    test('should return Right(void) when rejection is successful', () async {
      // Arrange
      when(mockRepository.rejectJoinRequest(testEventId, testUserId))
      ...
      verify(mockRepository.rejectJoinRequest(testEventId, testUserId)).called(1);
    ...
    test('should return Left(ValidationFailure) when eventId is empty', () async {
      ...
      verifyNever(mockRepository.rejectJoinRequest(any, any));
    });
    ...
    test('should return Left(ServerFailure) when repository returns failure', () async {
      // Arrange
      final failure = ServerFailure('Reject join request failed');
      when(mockRepository.rejectJoinRequest(testEventId, testUserId))
      ...
      expect(result.left.message, 'Reject join request failed');
      verify(mockRepository.rejectJoinRequest(testEventId, testUserId)).called(1);
    });
  });
}
