...
import 'package:thunder/features/event/domain/usecases/approve_join_request_usecase.dart';
import 'package:thunder/features/event/domain/repositories/event_repository.dart';
import 'package:thunder/core/errors/failures.dart';

import 'approve_join_request_usecase_test.mocks.dart';
...
void main() {
  ...
  setUp(() {
    ...
    useCase = ApproveJoinRequestUseCase(mockRepository);
  });

  group('ApproveJoinRequestUseCase', () {
    const testEventId = 'event-123';
    const testUserId = 'user-123';

    test('should return Right(void) when approval is successful', () async {
      // Arrange
      when(mockRepository.approveJoinRequest(testEventId, testUserId))
      ...
      verify(mockRepository.approveJoinRequest(testEventId, testUserId)).called(1);
    ...
    test('should return Left(ValidationFailure) when eventId is empty', () async {
      ...
      verifyNever(mockRepository.approveJoinRequest(any, any));
    });
    ...
    test('should return Left(ServerFailure) when repository returns failure', () async {
      // Arrange
      final failure = ServerFailure('Approve join request failed');
      when(mockRepository.approveJoinRequest(testEventId, testUserId))
      ...
      expect(result.left.message, 'Approve join request failed');
      verify(mockRepository.approveJoinRequest(testEventId, testUserId)).called(1);
    });
  });
}
