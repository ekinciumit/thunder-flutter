...
import 'package:thunder/features/chat/domain/usecases/search_messages_usecase.dart';
...
import 'search_messages_usecase_test.mocks.dart';

/// Mock classes için annotation
@GenerateMocks([ChatRepository])
void main() {
  late SearchMessagesUseCase useCase;
  late MockChatRepository mockRepository;

  setUp(() {
    mockRepository = MockChatRepository();
    useCase = SearchMessagesUseCase(mockRepository);
  });
  ...
  group('SearchMessagesUseCase', () {
    ...
    test('should return Right(List<MessageModel>) when search is successful', () async {
      ...
      // Act
      final result = await useCase.call(testChatId, testQuery, limit: 50);

      // Assert
      expect(result.isRight, true);
      expect(result.right, testMessages);
      verify(mockRepository.searchMessages(testChatId, testQuery, limit: 50)).called(1);
    });
    ...
    test('should return Left(ValidationFailure) when chatId is empty', () async {
      ...
      verifyNever(mockRepository.searchMessages(any, any, limit: anyNamed('limit')));
    });

    test('should return Left(ValidationFailure) when query is empty', () async {
      // Act
      final result = await useCase.call(testChatId, '');
    ...
    test('should return Left(ValidationFailure) when limit is zero', () async {
      // Act
      final result = await useCase.call(testChatId, testQuery, limit: 0);
    ...
    test('should return Left(ValidationFailure) when limit is greater than 200', () async {
      // Act
      final result = await useCase.call(testChatId, testQuery, limit: 201);

      // Assert
      expect(result.isLeft, true);
      expect(result.left, isA<ValidationFailure>());
      expect(result.left.message, 'Limit 200\'den küçük olmalıdır');
      verifyNever(mockRepository.searchMessages(any, any, limit: anyNamed('limit')));
    });

    test('should return Left(ServerFailure) when repository returns failure', () async {
      // Arrange
      final failure = ServerFailure('Search messages failed');
      when(mockRepository.searchMessages(any, any, limit: anyNamed('limit')))
          .thenAnswer((_) async => Either.left(failure));

      // Act
      final result = await useCase.call(testChatId, testQuery);

      // Assert
      expect(result.isLeft, true);
      expect(result.left, isA<ServerFailure>());
      expect(result.left.message, 'Search messages failed');
    });
  });
}
